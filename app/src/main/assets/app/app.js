  ...
  savedLocations.forEach((location) => {
    ...
    savedLocationItem
      ...
      .addEventListener("click", () => {
        ...
        if (
          localStorage.getItem(`DecodeWeatherLastCall_${location.locationName}`)
        ) {
          setTimeout(() => {
            localStorage.removeItem(
      ...
      if (
        ...
        setTimeout(() => {
          document.getElementById(
            "last_updated"
          ).innerHTML = `Updated, ${timeAgo(renderFromSavedDataMetTimstamp)}`;
        }, 1000);
      } else if (
        localStorage.getItem("ApiForAccu") &&
        localStorage.getItem("selectedMainWeatherProvider") === "Accuweather"
        ...
        setTimeout(() => {
          document.getElementById(
            "last_updated"
          ).innerHTML = `Updated, ${timeAgo(dataTimstamp)}`;
        }, 1000);
      } else {
        const renderFromSavedData = JSON.parse(
          localStorage.getItem(`WeatherDataOpenMeteo_${location.locationName}`)
...
function ReturnHomeLocation() {
  ...
  if (localStorage.getItem("selectedMainWeatherProvider") === "Met norway") {
    ...
    setTimeout(() => {
      document.getElementById("last_updated").innerHTML = `Updated, ${timeAgo(
        renderFromSavedDataMetTimstamp
      )}`;
    }, 1000);
  } else if (
    localStorage.getItem("ApiForAccu") &&
