...
function Circle(x, y, dx, dy, radius) {
   ...
   this.draw = function() {
      ...
      paintbrush.arc(this.x,this.y,this.radius,0,Math.PI*2, false);
...
for (var i = 0; i < 12; i++) {
   ...
   var radius = 40;
   // start point (randomised), also to not get circle caught at 0
   var x = Math.random() * (canvasWidth - radius * 2) + radius;
   var y = Math.random() * (canvasHeight - radius * 2) + radius;
   // x velocity (randomised)
   var randVelocity = Math.floor(getRandom(3, 6));
   var dx = (Math.random() - 0.5) * randVelocity;
   var dy = (Math.random() - 0.5) * randVelocity;
