...
val MIGRATION_55_56 = object : Migration(55, 56) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE api_keys ADD COLUMN savedAt INTEGER NOT NULL DEFAULT 0")
        // Backfill existing keys (e.g. Met Office) to "now" so they don't immediately
        // look expired under the new key-expiry check.
        db.execSQL("UPDATE api_keys SET savedAt = ${System.currentTimeMillis()}")
    }
}