...
class WeatherService {
  ...
  Future<Map<String, dynamic>?> fetchWeather(double lat, double lon,
    ...
    if (showAlerts && !isBackground) {
      final alertUri = Uri.parse('https://api.weatherapi.com/v1/alerts.json')
          .replace(queryParameters: {
  ...
  Map<String, dynamic> _mergeSection(
    ...
    for (final key in fallback.keys) {
      if (!merged.containsKey(key) || merged[key] == null) {
        ...
        final primaryHasValidData =
            primaryList.isNotEmpty && primaryList.any((value) => value != null);
        final fallbackHasValidData = fallbackList.isNotEmpty &&
            fallbackList.any((value) => value != null);
