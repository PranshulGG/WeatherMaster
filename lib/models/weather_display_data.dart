...
WeatherDisplayData processWeatherData(Map<String, dynamic> raw) {
  ...
  final hourlyWeatherCodes =
      filteredIndices.map((i) => hourlyWeatherCodesNoFilter[i]).toList();
  final hourlyPrecpProb =
      filteredIndices.map((i) => hourlyPrecpProbNoFilter[i]).toList();

  // Safe mapping for optional fields
  final hourlyDewPoint = filteredIndices
      .map((i) =>
          i < hourlyDewPointNoFilter.length ? hourlyDewPointNoFilter[i] : null)
      .toList();
  final hourlyVisibility = filteredIndices
      .map((i) => i < hourlyVisibilityNoFilter.length
          ? hourlyVisibilityNoFilter[i]
          : null)
      .toList();
  final hourlyUvIndex = filteredIndices
      .map((i) =>
          i < hourlyUvIndexNoFilter.length ? hourlyUvIndexNoFilter[i] : null)
      .toList();
  ...
  if (dailyDates.isNotEmpty &&
      sunriseTimes.isNotEmpty &&
      sunsetTimes.isNotEmpty) {
    for (int i = 0; i < dailyDates.length; i++) {
      if (i < sunriseTimes.length && i < sunsetTimes.length) {
        daylightMap[dailyDates[i]] =
            (DateTime.parse(sunriseTimes[i]), DateTime.parse(sunsetTimes[i]));
      }
    }
  }
