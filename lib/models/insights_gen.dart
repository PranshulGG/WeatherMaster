...
void _generateSmartInsight() {
  ...
  final random = Random();
  final selected = insightGenerators[random.nextInt(insightGenerators.length)];
...
String _coldTrendInsight() {
  final temps = widget.dailyData.map((d) => d['temperature_2m_max'] as double).toList();
  ...
  String _rainProbabilityInsight() {
    final rainChances = widget.dailyData.map((d) => d['precipitation_probability_max'] ?? 0).cast<num>().toList();
    final peakDayIndex = rainChances.indexOf(rainChances.reduce(max));
    final peakDay = DateTime.parse(widget.dailyData[peakDayIndex]['time']).weekday;
 ...
 final rawTemps = widget.dailyData.map((d) => d['temperature_2m_max']).toList();
  ...
  String _humidityTrendInsight() {
    final humidities = widget.dailyData.map((d) => d['relative_humidity_max'] ?? 0).cast<num>().toList();
  ...
  String _windInsight() {
    final winds = widget.dailyData.map((d) => d['wind_speed_max'] ?? 0).cast<num>().toList();
  ...
  String _pleasantWeatherInsight() {
    final sunnyDays = widget.dailyData.where((d) =>
  ...
  final temps = widget.hourlyData.map((d) => d['temperature_2m'] ?? 0).cast<num>().toList();
  final tempUnit = context.watch<UnitSettingsNotifier>().tempUnit;
...
String _uvIndexInsight() {
  final uv = widget.hourlyData.map((d) => d['uv_index'] ?? 0).cast<num>().toList();
...
String _windFluctuationInsight() {
  final winds = widget.hourlyData.map((d) => d['wind_speed_10m'] ?? 0).cast<num>().toList();
  final maxWind = winds.reduce(max);
...
String _hourlyHumidityInsight() {
  final humidities = widget.hourlyData.map((d) => d['relative_humidity_2m'] ?? 0).cast<num>().toList();
...
String _cloudPatternInsight() {
  final clouds = widget.hourlyData.map((d) => d['cloud_cover'] ?? 0).cast<num>().toList();
...
String _precipitationWindowInsight() {
  final precipChances = widget.hourlyData.map((d) => d['precipitation_probability'] ?? 0).cast<num>().toList();
