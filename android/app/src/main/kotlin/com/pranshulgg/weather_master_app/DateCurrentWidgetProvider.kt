...
class DateCurrentWidgetProvider : AppWidgetProvider() {
    override fun onUpdate(context: Context, manager: AppWidgetManager, ids: IntArray) {
        for (id in ids) {
            val views = RemoteViews(context.packageName, R.layout.date_current_temp_widget)
            ...
            views.setTextViewText(R.id.condition_current_time_date_temp, conditionName)
            views.setTextViewText(R.id.temp_time_temp_date_current, "$temp° • ")

            val iconRes = WeatherIconMapper.getIconResource(code, isDay)
            views.setImageViewResource(R.id.current_widget_temp_date_time_img, iconRes)
            ...
            views.setOnClickPendingIntent(R.id.widget_root_date_temp, pendingIntent)
